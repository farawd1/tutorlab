...
export function PasswordDialog({open,onOpenChange,api,onSessionToken}:{open:boolean;onOpenChange:(value:boolean)=>void;api:Api;onSessionToken:(token:string)=>void}){
 const [busy,setBusy]=useState(false);
 async function submit(event:React.FormEvent<HTMLFormElement>){event.preventDefault();const data=new FormData(event.currentTarget);setBusy(true);try{const response=await api("auth/password",{current:data.get("current"),next:data.get("next")});onSessionToken(response.sessionToken);toast.success("Пароль изменён");onOpenChange(false);}catch(e){toast.error(e instanceof Error?e.message:"Не удалось изменить пароль");}finally{setBusy(false);}}
